...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def unhandled_exception_handler(
        ...
        is_db_error = (
            isinstance(exc, db_error_types)
            or isinstance(getattr(exc, "__cause__", None), db_error_types)
            or isinstance(getattr(exc, "orig", None), db_error_types)
        )
        if is_db_error:
            return JSONResponse(
                status_code=503,
                content=_build_error_body(
                    ...
                    f"Database is unavailable: {exc.__class__.__name__}",
                ),
            )
        return JSONResponse(
            status_code=500,
