...
def cache_key_builder(
    ...
    *,
    ...
    For example: appointment-svc:get_visit:/api/visits/1
    """
...
async def invalidate_visit(visit_id: int) -> None:
    """Invalidate visit cache by ID."""
    if not redis_client:
        return
    key = f"{CACHE_PREFIX}:get_visit:/api/visits/{visit_id}"
