"""
Comprehensive API Test Script for Clinic Management System
...
def test_doctors(ids: dict):
    ...
    r = requests.post(f"{API}/doctors", json={
        ...
        "hourly_rate": 100.50, "phone": "+1234567890",
    ...
    # Second doctor for specialization filter test
    r2 = requests.post(f"{API}/doctors", json={
        "first_name": "James", "last_name": "Wilson",
        "specialization": "Oncologist",
        "hourly_rate": 120.00, "phone": "+1112223333",
    ...
    assert_field("GET /doctors returns list", {"data": r.json()}, "data")
    if r.status_code == 200 and isinstance(r.json(), list):
        ok("GET /doctors list length >= 2")
    ...
    r = requests.put(f"{API}/doctors/{ids['doctor_id']}", json={
        "first_name": "Gregory", "last_name": "House MD",
        "specialization": "Diagnostician",
        "hourly_rate": 110.00, "phone": "+1234567890",
        "email": "house@clinic.com"
    })
...
def test_patients(ids: dict):
    ...
    r = requests.post(f"{API}/patients", json={
        ...
        "phone": "+1987654321", "email": "john@example.com"
    })
    ...
    r = requests.put(f"{API}/patients/{ids['patient_id']}", json={
        ...
        "date_of_birth": "1990-05-15",
        "phone": "+1987654321", "email": "john.updated@example.com"
    })
...
def test_visits(ids: dict):
    section("VISITS")
    future = (datetime.datetime.utcnow() + datetime.timedelta(hours=2)).isoformat() + "Z"
    future2 = (datetime.datetime.utcnow() + datetime.timedelta(hours=4)).isoformat() + "Z"
    ...
    # --- COMPLETE on a non-existent visit → 404 ---
...
def test_diagnoses(ids: dict):
    ...
    future3 = (datetime.datetime.utcnow() + datetime.timedelta(hours=6)).isoformat() + "Z"
...
def test_payments(ids: dict):
    ...
    future4 = (datetime.datetime.utcnow() + datetime.timedelta(hours=8)).isoformat() + "Z"
    r_tmp = requests.post(f"{API}/visits", json={
        "patient_id": ids["patient_id"],
        "doctor_id": ids["doctor_id"],
...
def main():
    try:
        ...
        print("   Start the server: .\\venv\\Scripts\\uvicorn app.main:app --reload")
        sys.exit(1)
